...
public class Bleed extends Effect {
	...
	public void onApply(GameCharacter target) {
		...
		if (this.affected.getProfession().equals("Monster")) {
			message = "The " + affected.getRace()
					+ " begins bleeding from its wounds!";
		} else {
			message = affected.getName()
					+ " begins bleeding from their wounds!";
		}
	...
	public void tick() {
		...
		this.affected.stats.setCurrentHP(this.affected.stats.getCurrentHP()
				- damage);
		...
		if (this.affected.getProfession().equals("Monster")) {
			message = "The " + affected.getRace()
					+ "'s wounds bleed, draining " + damage + " health!";
		} else {
			message = affected.getName() + "'s wounds bleed, draining "
					+ damage + " health!";
		}
